...
async function getClientes() {
    try {
        let pool = await sql.connect(config);
        let lojas = await pool.request().query("SELECT * from Cliente");
        return lojas.recordsets;
    }
    catch (error) {
        console.log("aqui");
...
async function getCliente(ClienteId) {
    try {
        let pool = await sql.connect(config);
        let loja = await pool.request().input('input_parameter', sql.Int, ClienteId).query("SELECT * from [dbo].[Cliente] WHERE Id = @input_parameter");
        return loja.recordsets;
    }
    catch (error) {
        console.log("aqui2");
...
async function delClientes(ClienteId) {
    try {
        let pool = await sql.connect(config);
        let loja = await pool.request()
